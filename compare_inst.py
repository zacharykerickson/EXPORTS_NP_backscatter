...
if cf.SAVE_INFO and not path.exists(cf.SAVE_INFO_FN):
    f = open(cf.SAVE_INFO_FN,'a')
...
x = VSF_interp1.ravel()*2*np.pi*chi1
y = VSF_interp2.ravel()*2*np.pi*chi2
...
a_line_total_unc = np.sqrt(fit_line.sd_beta[0]**2 + fit_line.beta[0]**2 * (unc_sf1**2 + unc_sf2**2))
b_line_total_unc = np.sqrt(fit_line.sd_beta[1]**2 + (fit_line.beta[0]*(unc_dk1*2*np.pi*chi1))**2 +
                           (fit_line.beta[1]*unc_sf2)**2 + (unc_dk2*2*np.pi*chi2)**2)
b_offset_total_unc = np.sqrt(fit_offset.sd_beta[0]**2 + (unc_dk1*2*np.pi*chi1)**2 + (unc_dk2*2*np.pi*chi2)**2)
if cf.OUTPUT_INFO:
    print('Total error, adding in uncertainty in the calibration coefficients:')
    print('Best-fit  line:  b=%+.3f (%.3f) x1e-4, a=%.3f (%.3f)'
...
if cf.PLOT_FIG:
    ...
    ax[0].plot(XX,np.polyval(fit_line.beta,XX),color='k',
           ...
           %(r**2,fit_line.beta[0],CI_95_line_a,fit_line.beta[1]*1e4,CI_95_line_b*1e4))
    ax[0].plot(XX,XX+fit_offset.beta,color='k',ls='--',label='best-fit offset:\nb=%+.1f (%.1f) x10$^{-4}$ m$^{-1}$'
           %(fit_offset.beta[0]*1e4,CI_95_offset_b*1e4))
    ...
    if inst1=='FLBBLF':
        ax[0].set_xlim([0.001,0.003])
        ax[0].set_xticks(np.arange(0.001,0.0031,0.0005))
